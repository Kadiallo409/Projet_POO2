...
class login_page:
    def __init__(self, root):
        ...
        self.bg_img = ImageTk.PhotoImage(file="Images/cem.png")
        label = Label(self.frame1, image=self.bg_img).place(x=20, y=20)

        label1 = Label(self.frame1, text="JU", font=("times new roman", 40, "bold"), bg="yellow", fg="red").place(x=100,
                                                                                                                  y=300)
        label2 = Label(self.frame1, text="RY", font=("times new roman", 40, "bold"), bg="yellow",
                       fg="RoyalBlue1").place(x=162, y=300)
        label3 = Label(self.frame1, text="DELIBERATION", font=("times new roman", 13, "bold"), bg="yellow",
    ...
    def login_func(self):
        if self.email_entry.get() == "" or self.password_entry.get() == "":
            ...
            try:
                ...
                cur.execute("select * from Registation  where email=%s and password=%s",
    ...
    """def forgot_func(self):
        if self.email_entry.get()=="":
            ...
            try:
                connection = pymysql.connect(host="localhost", user="root",password="", database="jury")
                cur = connection.cursor()
                cur.execute("select * from Registation where email=%s", self.email_entry.get())
                row=cur.fetchone()
                if row == None:
    ...
    def change_pass(self):
        if self.email_entry.get() == "" or self.sec_ques.get() == "Select" or self.new_pass.get() == "":
            ...
            try:
                connection = pymysql.connect(host="localhost",user="root",password="",database="jury")
                cur = connection.cursor()
                cur.execute("select * from Registation where email=%s and question=%s and answer=%s", (self.email_entry.get(),self.sec_ques.get(),self.ans.get()))
                row=cur.fetchone()
                ...
                if row == None:
                    ...
                    try:
                        cur.execute("update Registation set password=%s where email=%s", (self.new_pass.get(),self.email_entry.get()))
                        connection.commit()
